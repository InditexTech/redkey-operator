...
stays in the `Ready` status (surfaced as `Substatus = Remediating`) and `Phase`/`Status` are **not**
downgraded — see [Phase, Status and Substatus](redkey-cluster-status.md#phase-status-and-substatus).
