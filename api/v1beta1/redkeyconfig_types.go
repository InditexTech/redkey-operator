...
// ConfigPhase constants for RedkeyConfig lifecycle.
const (
...
// Operational phase constants for RedkeyConfig status.
const (
...
// Health condition types set on RedkeyConfig by Robin from the cluster health report and
// aggregated onto the Redkey by the Operator. They expose live data-plane health
...
// RedkeyConfigSpec defines the desired state of RedkeyConfig.
// This spec is immutable after creation — set by the operator, read-only for Robin.
type RedkeyConfigSpec struct {
...
func (s RedkeyConfigSpec) IsStandalone() bool {
	return s.Mode == ModeStandalone
}

// RedkeyConfigStatus defines the observed state of RedkeyConfig.
// Written exclusively by Robin via the status subresource.
type RedkeyConfigStatus struct {
	...
	Substatus RedkeySubstatus `json:"substatus"`
...
// RedkeySubstatus defines detailed sub-status information.
type RedkeySubstatus struct {
	Status             string `json:"status"`
...
// +kubebuilder:resource:shortName=rkc
// +kubebuilder:printcolumn:name="Cluster",type="string",priority=0,JSONPath=".metadata.labels.redkey\\.inditex\\.dev/cluster",description="Parent Redkey"
...
// RedkeyConfig is the Schema for the redkeyconfigs API.
// It represents a sequenced desired-state snapshot created by the Operator and consumed by Robin.
type RedkeyConfig struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   RedkeyConfigSpec   `json:"spec,omitempty"`
	Status RedkeyConfigStatus `json:"status,omitempty"`
}

// +kubebuilder:object:root=true

// RedkeyConfigList contains a list of RedkeyConfig.
type RedkeyConfigList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []RedkeyConfig `json:"items"`
}

func init() {
	SchemeBuilder.Register(&RedkeyConfig{}, &RedkeyConfigList{})
}
