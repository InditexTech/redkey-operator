...
//   - Scale from >0 to 0: a RedkeyClusterConfig with primaries=0 is created for Robin to process.
...
//     (Robin Deployment, RBAC, all RedkeyClusterConfigs).
func (r *RedkeyClusterReconciler) reconcileScaleToZero(ctx context.Context, cluster *redisv1.RedkeyCluster) (ctrl.Result, error) {
...
func (r *RedkeyClusterReconciler) deleteRobinDeployment(ctx context.Context, cluster *redisv1.RedkeyCluster) error {
	log := logf.FromContext(ctx)
...
func (r *RedkeyClusterReconciler) deleteRBAC(ctx context.Context, cluster *redisv1.RedkeyCluster) error {
	log := logf.FromContext(ctx)
	name := fmt.Sprintf("%s-robin", cluster.Name)
...
// deleteAllConfigs deletes all RedkeyClusterConfigs associated with the cluster.
func (r *RedkeyClusterReconciler) deleteAllConfigs(ctx context.Context, cluster *redisv1.RedkeyCluster) error {
	log := logf.FromContext(ctx)
	...
	for i := range configList.Items {
		...
		log.Info("Deleted RedkeyClusterConfig", "config", configList.Items[i].Name)
	}
	return nil
}

// updateStatusScaledToZero updates the RedkeyCluster status to reflect a scaled-to-zero state.
func (r *RedkeyClusterReconciler) updateStatusScaledToZero(ctx context.Context, cluster *redisv1.RedkeyCluster) error {
	now := metav1.Now()

	cluster.Status.Replicas = 0
	cluster.Status.Phase = redisv1.PhaseReady
	cluster.Status.Status = ""
	cluster.Status.Substatus = redisv1.RedkeyClusterSubstatus{}
